...
import { createImapConnection as createYahooImapConnection } from './services/yahooMailService.js';
import { createMcpServer as buildMcpServer } from './services/mcpServerFactory.js';
import { createSetupRouter } from './routes/setup.js';
import {
    getConnectionStatus as getStoredConnectionStatus,
    getStoredConnectionForRuntime,
    removeSingleUserConnection,
    upsertSingleUserConnection,
...
class YahooMailMCPServer {
    ...
    isLegacyAppPasswordConfigured() {
        const { yahooEmail, yahooAppPassword } = getConfig();
        return Boolean(yahooEmail && yahooAppPassword);
    }

    isYahooOAuthConfigured() {
        const { yahooClientId, yahooClientSecret } = getConfig();
        return Boolean(yahooClientId && yahooClientSecret);
    }

    isMcpAuthorizationEnabled() {
        const { transportMode, publicBaseUrl, mcpStateSecret, sessionSecret, appEncryptionKey } = getConfig();
        return transportMode === 'sse' && Boolean(publicBaseUrl && (mcpStateSecret || sessionSecret || appEncryptionKey));
    }
    ...
    getLocalAuthContext() {
        ...
        return {
            mode: 'app_password',
            email: getConfig().yahooEmail,
            appPassword: getConfig().yahooAppPassword,
        };
    }

    getYahooScopes() {
        return getConfig().yahooScopes;
    }

    getExternalBaseUrl(req) {
        const configuredBaseUrl = getConfig().publicBaseUrl;
    ...
    getMcpStateSecret() {
        const { mcpStateSecret, sessionSecret, appEncryptionKey, yahooClientSecret } = getConfig();
        return mcpStateSecret || sessionSecret || appEncryptionKey || yahooClientSecret;
    }
    ...
    createSealedAccessToken({ clientId, yahooSession = null, authMode = 'yahoo_oauth', scope = MCP_SCOPE }) {
        const now = Date.now();
        const expiresAt = authMode === 'stored_app_password'
            ? now + (55 * 60 * 1000)
            : Math.min(
    ...
    createSealedRefreshToken({ clientId, yahooSession = null, authMode = 'yahoo_oauth', scope = MCP_SCOPE }) {
        const now = Date.now();
    ...
    parseSetupState(stateToken) {
        ...
        try {
            ...
            const payload = JSON.parse(Buffer.from(encodedPayload, 'base64url').toString('utf8'));
            if (!payload?.createdAt || (Date.now() - payload.createdAt) > (30 * 60 * 1000)) {
                return null;
            }
    ...
    async createStoredConnectionAuthorizationCode({ clientId, redirectUri, clientState, codeChallenge, codeChallengeMethod, scope = MCP_SCOPE }) {
        const storedConnection = await getStoredConnectionForRuntime();
        if (!storedConnection || storedConnection.status !== 'active') {
            throw new Error('Yahoo Mail is not connected yet.');
        }
    ...
    async exchangeYahooAuthorizationCode(code, redirectUri) {
        ...
        const { yahooClientId, yahooClientSecret } = getConfig();
        const authHeader = Buffer.from(`${yahooClientId}:${yahooClientSecret}`).toString('base64');
    ...
    issueMcpTokens({ clientId, yahooSession = null, authMode = 'yahoo_oauth', scope = MCP_SCOPE }) {
    ...
    async createImapConnection(authContext) {
        return createYahooImapConnection({
            authContext,
            resolveImapCredentials: async (context) => this.resolveImapCredentials(context),
        });
    }
    ...
    async run() {
        const transportMode = getConfig().transportMode;
    ...
    async runSSE() {
        ...
        console.error('[Server] Environment:', getConfig().nodeEnv);
        ...
        app.use('/setup', createSetupRouter({
            ...
            onConnect: async ({ yahooEmail, appPassword, setupState }) => {
                await upsertSingleUserConnection({ yahooEmail, appPassword });

                if (!setupState) {
                    return null;
                }

                const pendingSetup = this.parseSetupState(setupState);
                if (!pendingSetup) {
                    throw new Error('The pending Claude connection is invalid or expired. Please reconnect from Claude and try again.');
                }

                return await this.createStoredConnectionAuthorizationCode({
                    clientId: pendingSetup.clientId,
                    redirectUri: pendingSetup.redirectUri,
                    clientState: pendingSetup.clientState,
                    codeChallenge: pendingSetup.codeChallenge,
                    codeChallengeMethod: pendingSetup.codeChallengeMethod,
                    scope: pendingSetup.scope || MCP_SCOPE,
                });
            },
            onDisconnect: async () => {
                await removeSingleUserConnection();
            },
        }));
        ...
        const authenticateMcpRequest = (req, res, next) => {
            ...
            if (tokenRecord.authMode === 'stored_app_password') {
                getStoredConnectionForRuntime()
                    .then((storedConnection) => {
                        if (!storedConnection || storedConnection.status !== 'active') {
                            return sendAuthChallenge(
                                ...
                                res,
                                'invalid_token',
                                'No active Yahoo Mail connection is stored. Please reconnect the connector from Claude.'
                            );
                        }

                        req.authContext = {
                            mode: 'app_password',
                            email: storedConnection.yahooEmail,
                            appPassword: storedConnection.appPassword,
                        };
                        req.mcpToken = tokenRecord;
                        next();
                    })
                    .catch((error) => {
                        console.error('[Auth] Stored Yahoo credential lookup failed:', error.message);
                        return res.status(500).json({
                            error: 'server_error',
                            error_description: 'Unable to load the stored Yahoo Mail connection.',
                        });
                    });
                return;
            }
        ...
        app.get('/health', (req, res) => {
            res.json({
                ...
                environment: {
                    ...
                    transportMode: getConfig().transportMode,
            ...
            getStoredConnectionStatus()
                .then(async (connectionStatus) => {
                    if (connectionStatus.isConnected) {
                        const redirectTarget = await this.createStoredConnectionAuthorizationCode({
                    ...
                    const setupState = this.createSetupState({
                        clientId: client.clientId,
                        redirectUri: redirect_uri,
                        clientState: state,
                        codeChallenge: code_challenge,
                    ...
                    return res.redirect(`/setup?setup_state=${encodeURIComponent(setupState)}`);
                })
                .catch((error) => {
                    console.error('[OAuth] Setup handoff failed:', error);
                    return res.status(500).send('Unable to start connector setup right now. Please try again.');
                });
        };
        ...
        const handleToken = async (req, res) => {
            ...
            if (grantType === 'authorization_code') {
                ...
                return res.json(this.issueMcpTokens({
                    ...
                    authMode: authCode.authMode || 'yahoo_oauth',
            ...
            if (grantType === 'refresh_token') {
                ...
                if (refreshRecord.authMode === 'stored_app_password') {
                    const storedConnection = await getStoredConnectionForRuntime();
                    if (!storedConnection || storedConnection.status !== 'active') {
                        return res.status(400).json({
                            error: 'invalid_grant',
                            error_description: 'No active Yahoo Mail connection is stored. Please reconnect the connector.',
                        });
                    }
                ...
                return res.json(this.issueMcpTokens({
                    clientId: refreshRecord.clientId,
                    yahooSession,
                    authMode: refreshRecord.authMode || 'yahoo_oauth',
                    scope: refreshRecord.scope || MCP_SCOPE,
                }));
            }
